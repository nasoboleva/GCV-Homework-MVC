...
def pairwise_interpolate_predictions(
    ...
    for idx, point_from_j in tqdm(enumerate(reprojected_j)):
        ...
        image_i_indexed = image_i.flatten()[point_nn_indexes].reshape(-1, 1)
        point_from_j_nns = np.concatenate((uv_i[point_nn_indexes], image_i_indexed), axis=1)
        ...
        distance = np.sqrt(np.sum((point_from_j - point_from_j_nns) ** 2, axis=1))
        interp_mask[idx] = np.all(distance < distance_interpolation_threshold)
